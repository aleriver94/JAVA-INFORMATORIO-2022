...
public class Ejercicio5 {
    private static String bienvenida =
            "Bienvenido, dada una lista de objetos de tipo Alumno, obtendremos un Map con su nombre y apellido como clave y su edad como valor.";

    public static LocalDate obtenerFecha(int i) {
        LocalDate fechaN = Calendar.getInstance().getTime().toInstant()
                .atZone(ZoneId.systemDefault()).toLocalDate().minusYears(i);
        return fechaN;
    }

    public static void generarMap(List<Alumno> a) {
        Map<String, Integer> alumnosMap = a.stream().collect(Collectors.toMap(
                (x -> "\"" + x.getApellido() + " " + x.getNombre() + "\""), x -> x.getEdad()));
        System.out.println(alumnosMap);
    }

    public static void main(String[] args) {
        System.out.println(bienvenida);
        List<Alumno> alumnos = List.of(new Alumno("Homero", "Simpson", obtenerFecha(35)),
